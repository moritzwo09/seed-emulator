#!/usr/bin/env python3
"""Generate the KVM + Kube-OVN/OVS K8sPre example.

The script writes setup/ and running/ directories. It does not create VMs,
install K3s, download image caches, build images, or deploy workloads.
"""
...
CONFIG_PATH = SCRIPT_DIR / "configKvmOvn.yaml"
...
def writeKvmOvnExample(output_root: Path, compile_output: Path) -> None:
    """Write setup/running scripts for the KVM + Kube-OVN/OVS flow."""
    k8spre = K8sPre()
    ...
    writeSeedemuDockerDir(setup_dir / "kvm.yaml")
    ...
    print("KVM + Kube-OVN/OVS example generated.")
