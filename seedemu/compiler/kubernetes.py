...
class KubernetesCompiler(Docker):
    ...
    def _compileNetK8s(self, net: Network) -> str:
        ...
        - kube-ovn/ovn: Kube-OVN attached networks through OVS
        """
        ...
        scope, _, registry_name = net.getRegistryInfo()
        annotations = {
            "org.seedsecuritylabs.seedemu.meta.type": "global" if scope == "ix" else "local",
            "org.seedsecuritylabs.seedemu.meta.scope": str(scope),
            "org.seedsecuritylabs.seedemu.meta.name": str(registry_name),
            "org.seedsecuritylabs.seedemu.meta.prefix": prefix,
        }
        if net.getDisplayName() is not None:
            annotations["org.seedsecuritylabs.seedemu.meta.displayname"] = str(net.getDisplayName())
        if net.getDescription() is not None:
            annotations["org.seedsecuritylabs.seedemu.meta.description"] = str(net.getDescription())
    ...
    def _compileNodeK8s(self, node: Node) -> str:
        ...
        virtualization_mode = getattr(node, "getVirtualizationMode", lambda: "Container")()
        if virtualization_mode == "KubeVirt":
...
class NativeKubernetesCompiler(KubernetesCompiler):
    """Compatibility name kept for callers using the native compiler import."""
