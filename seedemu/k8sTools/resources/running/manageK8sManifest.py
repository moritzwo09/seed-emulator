...
def render_kustomization(args: argparse.Namespace) -> None:
    ...
    if network_backend in {"kube-ovn", "ovn"}:
        ...
        if manifest_path.resolve() != rendered_manifest.resolve():
            renderKubeOvnManifest(args.manifest, rendered_manifest)
...
def renderKubeOvnManifest(manifest_path: str, output_path: Path) -> None:
    """Render a SeedEMU manifest that uses Kube-OVN secondary L2 networks.
    ...
    with static `ips` values in CIDR form. Kube-OVN attached NICs use provider
    annotations such as `<nad>.<namespace>.ovn.kubernetes.io/ip_address` and
    expect plain IP strings. The renderer therefore converts NADs to
    type=kube-ovn, creates matching Subnets, and rewrites Pod template
    annotations without changing the compiler output.
    """
    ...
    for doc in docs:
        if doc.get("kind") == "Namespace" and not vpc_written:
            rendered.append(doc)
            rendered.append(kubeOvnVpc(namespace_name, vpc_name))
            vpc_written = True
            continue
        if doc.get("kind") == "NetworkAttachmentDefinition":
            converted = convertNetworkAttachmentToKubeOvn(doc, namespace_name, vpc_name)
            rendered.extend(converted)
            continue
        convertWorkloadAnnotationsToKubeOvn(doc, namespace_name)
        rendered.append(doc)
...
def convertWorkloadAnnotationsToKubeOvn(doc: dict[str, Any], default_namespace: str) -> None:
    ...
    for macvlan with static IPAM. Kube-OVN non-primary mode uses provider
    annotations and expects plain IP values, so each `ips` item is converted to
    `<nad>.<namespace>.ovn.kubernetes.io/ip_address: 10.x.x.x`.
    """
